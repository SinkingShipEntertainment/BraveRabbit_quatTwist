...
uuid = "repository.BraveRabbit_quatTwist"
...
def commands():
    ...
    env.QUATTWIST_VERSION = external_version
    env.QUATTWIST_PACKAGE_VERSION = external_version
    if internal_version:
        env.QUATTWIST_PACKAGE_VERSION = internal_version

    env.QUATTWIST_ROOT.append("{root}")
    env.QUATTWIST_LOCATION.append("{root}")

    env.LD_LIBRARY_PATH.append("{root}/quatTwist/plug-ins")
    env.PYTHONPATH.append("{root}/quatTwist/scripts")

    # For Maya to locate the .mod file to setup env variables for plugins and libraries
    env.MAYA_MODULE_PATH.append("{root}")

    env.MAYA_SCRIPT_PATH.append("{root}/quatTwist/script")
